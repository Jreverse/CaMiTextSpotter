...
from detectron2.modeling.roi_heads.multilingual.language.languages import get_language_config, lang_code_to_char_map_class
from detectron2.modeling.roi_heads.multilingual.language.chars import char2num
...
class DynamicHead(nn.Module):
    ...
    def __init__(self, cfg, roi_input_shape):
        ...
        if cfg.MODEL.SWINTS.HEAD.FREEZE:
            print("[Info] Freezing DynamicHead Layers. W/O rec_stage.")
            self._freeze()

    def _freeze(self,):
        for key, param in self.named_parameters():
            if 'rec_stage' in key:
                # freeze rec_stage by cfg.MODEL.REC_HEAD.FREEZE
                continue
    ...
    def extra_rec_feat(self, matcher, mask_encoding, targets, N, bboxes, class_logits, pred_bboxes, mask_logits, proposal_features, features):
        ...
        if targets:
            ...
            if self.cfg.SEQUENCE.MULTI:
                target_rec = torch.cat([torch.zeros(len(t['text'][i]),100) for t, (_, i) in zip(targets, indices)], dim=0)
                
                target_text = []
                target_lang = []
                for t, (_, i) in zip(targets, indices):
                    tmp_text = [t['text'][i]] if len(i) == 1 else t['text'][i]
                    tmp_lang = [t['language'][i]] if len(i) == 1 else t['language'][i]
                    target_text.extend(tmp_text)
                    target_lang.extend(tmp_lang)
                target_text = np.concatenate([target_text*2], axis=0)
                target_lang = np.concatenate([target_lang*2], axis=0)
        ...
        # TODO: check self.cfg.MODEL.REC_HEAD.BATCH_SIZE
        if targets:
            if self.cfg.SEQUENCE.MULTI:
                return proposal_features, gt_masks[:self.cfg.MODEL.REC_HEAD.BATCH_SIZE], idx, rec_map, target_rec[:self.cfg.MODEL.REC_HEAD.BATCH_SIZE], target_text[:self.cfg.MODEL.REC_HEAD.BATCH_SIZE], target_lang[:self.cfg.MODEL.REC_HEAD.BATCH_SIZE]
            else:
                return proposal_features, gt_masks[:self.cfg.MODEL.REC_HEAD.BATCH_SIZE], idx, rec_map, target_rec[:self.cfg.MODEL.REC_HEAD.BATCH_SIZE], None, None
        else:
    ...
    def convert_seq_dict(
        ...
        text_target = torch.zeros((self.MAX_LENGTH,), dtype=torch.long)
        # TODO: maybe need support MAX_LENGTH=64 or 100
        for i, char in enumerate(encode_text):
            ...
            label = char2num(
                    ...
                    max_char_num=num_chars,
                    char_map_class=char_map_class,
                )
                    
            assert label >= 0 and label <= num_chars, (
                    "[Error] char2num:"
                    + " char: {},".format(char)
    ...
    def forward(self, features, init_bboxes, init_features, targets = None, mask_encoding = None, matcher=None):
        ...
        N, nr_boxes = bboxes.shape[:2]
        if targets:
            proposal_features, gt_masks, idx, rec_map, target_rec, target_text, target_lang = \
            ...
            if self.cfg.SEQUENCE.MULTI:
                gt_text_dict = {}
                for encode_language in sorted(set(self.cfg.SEQUENCE.LANGUAGES + self.cfg.SEQUENCE.LANGUAGES_ENABLED)):
                    ...
                    if encode_language not in self.language_config:
                        lang_cfg = get_language_config(self.cfg, encode_language)
                        self.language_config[encode_language] = lang_cfg
                    else:
                        lang_cfg = self.language_config[encode_language]
                    
                    gt_text_list = []
                    assert len(target_lang) == len(target_text)
                    for gt_lang, gt_text in zip(target_lang, target_text):
                        # print('gt_text: '+gt_text)
                        # print('gt_lang: '+gt_lang)
                        # print('encode_language: '+encode_language)
                        if self.only_generic_training:
                            # force to trans from any to any
                            gt_lang = 'any'
                        gt_text = self.convert_seq_dict(encode_text=gt_text,
                                                            encode_language=encode_language,
                                                            num_chars=lang_cfg.NUM_CHAR,
                                                            gt_language=gt_lang,
                                                            char_map_class=char_map_class)
                        # print(gt_text)
                        # language='none' text=‘###’会被编码，ocr识别的时候需要剔除none样本
                        gt_text_list.append(gt_text)
                    
                    # TODO: bug 3
                    # handle: by pass instances=0 samples in dataloader
                    gt_text_array = torch.stack(gt_text_list).to(device=proposal_features.device)
                    gt_text_dict[encode_language] = gt_text_array
                    
                    assert gt_text_array.size(0)==len(target_text), 'text to tensor dict not match!'
                    
                target_text = gt_text_dict
        else:
        ...
        if targets:
            rec_result = self.rec_stage(rec_map, rec_proposal_features, gt_masks, N, nr_boxes, idx, target_rec, target_text, target_lang)
        else:
...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler):
        """
        :param features: [1/4 features, 1/8 features, 1/16 features, 1/32 features]
