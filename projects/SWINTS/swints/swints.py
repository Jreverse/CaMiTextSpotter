...
class SWINTS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mutlingual_pred_on = self.cfg.SEQUENCE.MULTI
        ...
        # TODO: loss weight 
        if cfg.SEQUENCE.MULTI:
            # for multi_rec weight
            for language in self.cfg.SEQUENCE.LANGUAGES_ENABLED:
                key = 'loss_seq_{}'.format(language)
                weight_dict[key] = 1.0
            # for script cls weight
            weight_dict['loss_script_cls'] = 1.0
            # loss kye dict
            if self.cfg.MODEL.REC_HEAD.E2ETrain:
                losses = ["labels", "boxes", "masks", "multi_rec",]
            elif self.only_generic_training:
                losses = ["labels", "boxes", "masks", "multi_rec",]
            else:
                losses = ["labels", "boxes", "masks", "multi_rec", "cls"]
        else:
        ...
        if cfg.MODEL.SWINTS.FREEZE:
            self._freeze()
            
            print('[Info] Freeze on, layers can get grads as follows:')
            for k, p in self.named_parameters():
                if p.requires_grad:
                    print('key: {}, required_grad:{}'.format(k, str(p.requires_grad)))

    def _freeze(self,):
        # freeze pos_embeddings and so on
        for key, param in self.named_parameters():
            if 'pos_embeddings' in key:
                print("[Info] Freezing SWINTS.pos_embeddings Layers.")
                param.requires_grad=False
            if 'init_proposal_boxes' in key:
                print("[Info] Freezing SWINTS.init_proposal_boxes Layers.")
                param.requires_grad=False
            if 'backbone' in key or 'head' in key:
                # freeze backbone using cfg.MODEL.BACKBONE.FREEZE
                # freeze head using cfg.MODEL.SWINTS.HEAD.FREEZE
                continue
    ...
    def forward(self, batched_inputs):
        ...
        # print(batched_inputs[0]['instances'].get('text'))
        # print(batched_inputs[0]['instances']._nonempty)
        ...
        if self.training:
            ...
            # debug for loss.loss_boxes
            # if 'corner_case' in loss_dict:
            #     global batch_num
            #     batch_num += 1
            #     logger.info('find corner case, batch num: {}'.format(str(batch_num)))
            #     logger.info('batch images: '+' '.join([t['file_name'] for t in batched_inputs]))
            #     del_list = []
            ...
            if self.mutlingual_pred_on:
                results.seq_outputs_list = np.array(out_rec['seq_outputs_list']).transpose(1,0)
                results.seq_scores_list = out_rec['seq_scores_list'].permute(1,0,2)
                # results.detailed_seq_scores_list = out_rec['detailed_seq_scores_list']
                results.language_probs = out_rec['language_probs']
            else:
                results.pred_rec = out_rec['pred_rec']
            results = [results]
            processed_results = []
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            if self.cfg.SEQUENCE.MULTI:
                target["text"] = targets_per_image.text
                target["language"] = targets_per_image.language
            else:
    ...
    @torch.no_grad()
    def inference(self, box_cls, box_pred, mask_pred, image_sizes, rec_pred):
        """
        Arguments:
