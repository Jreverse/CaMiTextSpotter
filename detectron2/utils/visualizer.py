...
from detectron2.structures import BitMasks, Boxes, BoxMode, Keypoints, PolygonMasks, RotatedBoxes, WordResult
...
class Visualizer:
    ...
    def __init__(self, cfg, img_rgb, metadata=None, scale=1.0, instance_mode=ColorMode.IMAGE):
        """
        Args:
        ...
        """
        self.cfg = cfg
        self.multi = self.cfg.SEQUENCE.MULTI
        ...
        # TODO: 考虑是不是要把转换text的代码放到 SWINTS类里面，同时evaluation部分可能需要调整
        
        if self.multi:
            self.multilangual_translator = MultilingualTranslator(self.cfg)
    ...
    def draw_instance_predictions(self, predictions, path):
        ...
        if self.multi:
            rec = predictions.seq_outputs_list
            rec_score = predictions.seq_scores_list
            rec_lang = predictions.language_probs
        else:
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            ...
            alpha = 0.3
        if self.multi:
            self.overlay_instances_multilingual(
                rec=rec,
                rec_score = rec_score,
                rec_lang = rec_lang,
                masks=masks,
    ...
    def overlay_instances_multilingual(
        self,
        *,
        rec=None,
        boxes=None,
        ...
        if areas is not None:
            ...
            rec_score = [rec_score[idx] for idx in sorted_idxs] if rec_score is not None else None
            rec_lang = [rec_lang[idx] for idx in sorted_idxs] if rec_lang is not None else None
        ...
        words, seq_scores, languages, language_probs = self.multilangual_translator(rec, rec_score, rec_lang)
        
        rec = ['{},{:.1f}'.format(w, s)for w, s in zip(words, seq_scores)]
        lang = ['{},{:.1f}'.format(l, lp) for l, lp in zip(languages, language_probs)]
    ...
    def draw_text(
        ...
        *,
        ...
        self.output.ax.text(
            ...
            color=color,
            zorder=10,
            fontproperties=FontProperties(fname='fonts/Arial-Unicode-Regular.ttf'),
            rotation=rotation,
            #luyao
            # fontweight='light'
        )
