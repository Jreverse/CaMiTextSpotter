...
def annotations_to_instances(annos, image_size, mask_format="polygon", multi=False):
    """
    ...
    if multi:
        # for multisints
        text = [obj["text"] for obj in annos]
        target.text = np.array(text)
        language = [obj["language"] for obj in annos]
        target.language = np.array(language)
    else:
...
def filter_empty_instances(instances, by_box=True, by_mask=True, box_threshold=1e-5):
    ...
    instances._nonempty = [idx for idx, value in enumerate(m) if value]
    return instances[m]
