...
from detectron2.evaluation.mlt19.utils import MLT19Utils as utils
from detectron2.evaluation.utils.weighted_editdistance import weighted_edit_distance
from detectron2.modeling.roi_heads.multilingual.language.languages import lang_code_to_char_map_class, code_to_name, name_to_code
import os
...
def bbox_closewise(results):
    for res in results:
        bbox = res["polygon"]
        # bbox clockwise sort
        bbox = np.array(bbox).reshape((-1, 2)).astype(np.float32)
        rect = cv2.minAreaRect(bbox)
        bbox = cv2.boxPoints(rect)
        bbox = np.int0(bbox).reshape(-1).tolist()
        res["polygon"] = bbox
    return results
...
def process_res_files(
    ...
    for res_file in all_files:
        ...
        filtered_results = bbox_closewise(filtered_results)
...
def prepare_results_for_evaluation(
    ...
    if char_map_version != "none":
        char_map_dir = (
            f"charmap/public/{char_map_version}/"
        )
    ...
    # process_res_files_partial(all_files)
